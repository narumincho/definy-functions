      ...
      case "google": {
        return tool.urlFromStringWithQuery(
          ...
          new Map([
            ...
            ["state", await database.generateAndWriteLogInState("google")]
          ])
        );
      }
      case "gitHub": {
        return tool.urlFromStringWithQuery(
          ...
          new Map([
            ...
            ["scope", "read:user"],
            ["state", await database.generateAndWriteLogInState("gitHub")]
          ])
        );
      }
      case "line":
        return tool.urlFromStringWithQuery(
          ...
          new Map([
            ...
            ["scope", "profile openid"],
            ["state", await database.generateAndWriteLogInState("line")]
          ])
        );
    }
  },
  ...
  fields: () =>
    makeObjectFieldMap<type.Commit>({
      ...
      projectIcon: makeObjectField({
        type: g.GraphQLNonNull(type.hashGraphQLType),
        description: "プロジェクトのアイコン画像 " + type.fileHashDescription,
        args: {},
        resolve: async (source, args) => {
      ...
      projectImage: makeObjectField({
        type: g.GraphQLNonNull(type.hashGraphQLType),
        description:
          "プロジェクトのパッケージデザイン " + type.fileHashDescription,
        args: {},
        resolve: async (source, args) => {
  ...
  fields: () =>
    makeObjectFieldMap<type.DraftCommit>({
      ...
      projectDescription: makeObjectField({
        ...
        resolve: async (source, args) => {
          if (source.projectDescription === undefined) {
            return (await setDraftCommit(source)).projectDescription;
          }
          return source.projectDescription;
        }
      }),
...
> = new g.GraphQLObjectType({
  ...
  fields: () =>
    makeObjectFieldMap<type.ModuleSnapshot>({
      hash: {
        type: g.GraphQLNonNull(type.idGraphQLType),
        description: "モジュールのスナップショットから導き出されるハッシュ値"
      },
      name: makeObjectField({
        type: g.GraphQLNonNull(
          g.GraphQLList(g.GraphQLNonNull(type.labelGraphQLType))
        ),
        description: "モジュールの名前 リストをパスとして階層構造となす。",
        args: {},
        resolve: async (source, args) => {
          if (source.name === undefined) {
...
>({
  ...
  fields: () =>
    makeObjectFieldMap<type.PartDefSnapshot>({
      hash: {
        type: g.GraphQLNonNull(type.idGraphQLType),
        description: "パーツののスナップショットから導き出されるハッシュ値"
      },
      name: makeObjectField({
        type: g.GraphQLNonNull(type.labelGraphQLType),
        description: "パーツの名前",
